<div>
    <div class="flex items-center justify-between mb-4">
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('Search suppliers...') }}" class="input-field w-64" />
        <button wire:click="openCreate" class="btn-primary flex items-center gap-2">
            <x-pos-icon name="plus" class="w-4 h-4" /> {{ __('Add Supplier') }}
        </button>
    </div>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($suppliers as $s)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-gray-500">{{ $s->company ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-500">{{ $s->email ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-500">{{ $s->phone ?? '—' }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="badge-{{ $s->is_active ? 'green' : 'red' }}">{{ $s->is_active ? __('Active') : __('Inactive') }}</span>
                        </td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button wire:click="openEdit({{ $s->id }})" class="btn-icon-edit">
                                    <x-pos-icon name="pencil" class="w-4 h-4" />
                                </button>
                                <button wire:click="confirmDelete({{ $s->id }})" class="btn-icon-delete">
                ...
                @empty
                    <tr><td colspan="6" class="px-4 py-12 text-center text-gray-400">{{ __('No suppliers found.') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="modal-overlay" wire:click.self="$set('showModal',false)">
        <div class="modal-box max-w-lg w-full">
            <h3 class="modal-title">{{ $editingId ? __('Edit Supplier') : __('New Supplier') }}</h3>
            <div class="grid grid-cols-2 gap-4 mt-4">
                ...
                <div>
                    ...
                    <input wire:model="company" type="text" class="input-field w-full" />
                ...
                <div class="flex items-center gap-2 mt-4">
                    <input wire:model="is_active" type="checkbox" id="sup_active" class="w-4 h-4" />
                    <label for="sup_active" class="text-sm">{{ __('Active') }}</label>
                </div>
                <div class="col-span-2">
    ...
    <div class="modal-overlay">
        <div class="modal-box max-w-sm w-full text-center">
            ...
            <h3 class="text-lg font-semibold">{{ __('Delete Supplier?') }}</h3>
